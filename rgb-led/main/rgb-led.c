...
void app_main(void) {
  ...
  // Enable task watchdog for this task
  ...
  while (1) {
    ...
    if (gpio_get_level(PUSH_BUTTON) == 1) {
      ESP_LOGI("ledc_btn", "Pressed!");
      PRESSED = !PRESSED;
      vTaskDelay(1000 / portTICK_PERIOD_MS);
    }

    if (PRESSED != PREV_PRESSED) {
      setColor(sample_colors[rand() % 6]);
      PREV_PRESSED = PRESSED;
    } else if (!PRESSED && !PREV_PRESSED) {

      for (int i = 0; i < 3; i++) {
        ESP_ERROR_CHECK(
            ledc_set_duty(LEDC_LOW_SPEED_MODE, ledc_channel[i].channel, 0));
        ESP_ERROR_CHECK(
            ledc_update_duty(LEDC_LOW_SPEED_MODE, ledc_channel[i].channel));
      }
    }
